...
def setup_azure_openai():
    ...
    if not all([api_key, endpoint, deployment_chat, deployment_embedding, api_version]):
        st.error("⚠️ Assicurati che tutte le variabili di ambiente Azure OpenAI siano configurate nel file .env")
        st.stop()
...
def build_faiss_vectorstore(chunks: List[Document], embeddings_func: AzureOpenAIEmbeddings, persist_dir: str) -> FAISS:
    """
    ...
    with st.sidebar:
        with st.spinner("🔄 Processando documenti..."):
            try:
                ...
                vector_store = build_faiss_vectorstore(chunks, embeddings, settings.persist_dir)
                ...
                st.success(f"✅ Processati {len(all_documents)} documenti in {len(chunks)} chunks!")
                return True
